// Ported from Enemies.as
import { gs } from '../core/GameState.ts';
import { textL, textLP } from '../ui/TextRenderer.ts';
import { percent } from '../core/GameUtilities.ts';
import { setEnemyStats, doeHP, doeLust, doBattle, eDmg } from '../systems/Battling.ts';
import { doLust, doHP } from '../systems/StatChanges.ts';
import { stats } from '../systems/StatSetup.ts';
import {
  vulvaDesc, legDesc, cockDesc, nipDesc,
  buttDesc, hipDesc, boobDesc, ballDesc, clitDesc, skinDesc, udderDesc, teatDesc,
  plural, tailDesc, oneYour, legVerb, legWhere, bellyDesc
} from '../content/Descriptions.ts';
import { clothesBottom, clothesTop, pullUD, changeTop, changeBot } from '../content/Clothes.ts';
import { moistCalc, vagLimit, eVagLimit, cumAmount, milkAmount } from '../core/Calculations.ts';
import { vagChange, cockChange, boobChange } from '../systems/Transformations.ts';
import { doImpregnate, pregCheck } from '../systems/Pregnancy.ts';

// ── enemy name lookup ──────────────────────────────────────────────────────

export function enemyName(): string {
  let tempStr = 'ENEMY NAME ERROR';
  if (gs.enemyID === 101) { tempStr = 'cock-snake'; }
  if (gs.enemyID === 102) { tempStr = 'desiccating dust devil'; }
  if (gs.enemyID === 201) { tempStr = 'lone male wolf'; }
  if (gs.enemyID === 202) { tempStr = 'gay wolf'; }
  if (gs.enemyID === 301) { tempStr = 'felin in heat'; }
  if (gs.enemyID === 302) { tempStr = 'drunken equan'; }
  if (gs.enemyID === 303) { tempStr = 'octopus girl'; }
  if (gs.enemyID === 304) { tempStr = 'little big bunny-man'; }
  if (gs.enemyID === 305) { tempStr = 'little big bunny-girl'; }
  if (gs.enemyID === 306) { tempStr = 'fierce naga'; }
  if (gs.enemyID === 307) { tempStr = 'Minotaur'; }
  if (gs.enemyID === 308) { tempStr = 'freaky little girl'; }
  if (gs.enemyID === 309) { tempStr = 'succubus'; }
  return tempStr;
}

// ── enemy base stats ───────────────────────────────────────────────────────

export function enemyBaseStats(): void {
  // setEnemyStats(HP,str,ment,sen,lib,lust,gen,pref,coin,XP,item)
  if (gs.enemyID === 101) { setEnemyStats(30, 16, 4, 8, 0, 0, 0, 4, 0, 15, 202); }
  if (gs.enemyID === 102) { setEnemyStats(25, 0, 20, 0, 50, 0, 0, 0, 0, 10, 231); }
  if (gs.enemyID === 201) { setEnemyStats(45, 22, 16, 13, 11, 30, 1, 2, 0, 20, 203); }
  if (gs.enemyID === 202) { setEnemyStats(45, 26, 16, 20, 11, 40, 1, 1, 0, 20, 203); }
  if (gs.enemyID === 301) { setEnemyStats(50, 12, 10, 24, 30, 40, 2, 4, Math.floor(percent() / 10), 25, 204); }
  if (gs.enemyID === 302) { setEnemyStats(60, 28, 9, 18, 14, 30, 1, 4, Math.floor(percent() / 10), 25, 205); }
  if (gs.enemyID === 303) { setEnemyStats(150, 45, 30, 25, 35, 20, 2, 4, 0, 50, 216); }
  if (gs.enemyID === 304) { setEnemyStats(55, 35, 30, 35, 45, 10, 1, 4, Math.floor(percent() / 10), 30, 222); }
  if (gs.enemyID === 305) { setEnemyStats(50, 35, 30, 45, 35, 10, 2, 4, Math.floor(percent() / 10), 30, 222); }
  if (gs.enemyID === 306) { setEnemyStats(100, 50, 20, 40, 2, 40, 2, 4, Math.floor(percent() / 5), 55, 230); }
  if (gs.enemyID === 307) { setEnemyStats(250, 70, 20, 50, 20, 10, 1, 4, Math.floor(percent() / 4 + 5), 50, 525); }
  if (gs.enemyID === 308) { setEnemyStats(175, 80, 40, 70, 60, 10, 2, 4, Math.floor(percent() / 4 + 5), 55, 259); }
  if (gs.enemyID === 309) { setEnemyStats(150, 35, 80, 40, 40, 0, 2, 4, Math.floor(percent() / 4 + 5), 60, 260); }
}

// ── enemy baby race for pregnancy ─────────────────────────────────────────

export function enemyBaby(): number {
  let tempNum = 0;
  if (gs.enemyID === 201) { tempNum = 100; }
  if (gs.enemyID === 302) { tempNum = 2; }
  if (gs.enemyID === 304) { tempNum = 7; }
  if (gs.enemyID === 307) { tempNum = 307; }
  if (gs.enemyID === 308) { tempNum = 308; }
  return tempNum;
}

// ── special win text (rape) ────────────────────────────────────────────────

export function specialRapeWin(): void {
  if (gs.enemyID === 303) {
    textLP('\r\rThe octopus shudders as her belly quakes, the orgasm having wracked her a little too strongly. She huffs as she begins to crawl back towards the ocean. Just as she touches the water, she lets out a loud groan. Her tentacles quiver and part, exposing her vaginal-beak. The thing yawns wide as a spherical object pushes through.\r\rClear with a solid pink center, the egg falls from her dripping pussy-beak, and into the sand. Barely 4 inches in diameter, it doesn\'t come close to the size of her belly. She quickly catches her breath, her pink face red with embarrassment, before she pushes herself into the water, a tentacle grabbing the egg and pulling it with her before any more can escape...\r\rHowever, she seems to have left behind a big gob of pink goop.');
  } else if (gs.enemyID === 304 || gs.enemyID === 305) {
    textLP('\r\rAs the ' + enemyName() + ' collapses to the ground, wracked by orgasms and thoroughly distracted by the lewd fun, you find yourself beginning to grow. The effects of the bottle seems to wear off as you return to your normal height. Donning your outfit once again, however, it feels a little loose. It seems as though you\'re still missing a couple inches in height... On the other hand, however, inspecting the ' + enemyName() + '\'s body, you find something wedged in a rather lewd location.');
  } else if (gs.enemyID === 306) {
    textLP('\r\rThe naga collapses to the ground, tail still squirming in delighted orgasm and no longer wishing to battle. You take a moment to brush off some of the sparkly colorful dust from her wings for potential use later. Not quite sparkly of colorful when in your hands, you can at least ball it up into a nice suppository.');
  } else if (gs.enemyID === 307) {
    ...
    if (gs.defeatedMinotaur === false) {
      gs.defeatedMinotaur = true;
      gs.currentDungeon = 1003;
    } else {
      gs.currentDungeon = 1003;
    }
  } else if (gs.enemyID === 308) {
    ...
    if (gs.defeatedFreakyGirl === false) {
      gs.defeatedFreakyGirl = true;
      gs.currentDungeon = 1006;
    } else {
      gs.currentDungeon = 1006;
    }
  } else if (gs.enemyID === 309) {
    ...
    if (gs.defeatedSuccubus === false) {
      gs.defeatedSuccubus = true;
      gs.currentDungeon = 1010;
    } else {
      gs.currentDungeon = 1009;
    }
  } else {
    textLP('\r\rThe ' + enemyName() + ' collapses to the ground, quivering body wracked by orgasms. Having broken its mental state into a puddle of sex, it no longer wishes to battle.');
  }
}

// ── special win text (KO) ─────────────────────────────────────────────────

export function specialKOWin(): void {
  if (gs.enemyID === 303) {
    textLP('\r\rSaddened with your rough treatment, you don\'t seem to want to play like she does. A bit hurt, she jumps back into the ocean, leaving nothing but a gob of pink goop behind.');
  } else if (gs.enemyID === 304 || gs.enemyID === 305) {
    textLP('\r\rAs the ' + enemyName() + ' passes out from all the pain, you find yourself beginning to grow. The effects of the bottle seems to wear off as you return to your normal height. Donning your outfit once again, however, it feels a little loose. It seems as though you\'re still missing a couple inches in height... On the other hand, however, inspecting the ' + enemyName() + '\'s body, you find something wedged in a rather lewd location.');
  } else if (gs.enemyID === 102) {
    textLP('\r\rFeeling itself about to dissipate from your attacks, the desiccating dust devil feebly spins away, leaving behind some of the sand it can no longer support.');
  } else if (gs.enemyID === 306) {
    textLP('\r\rThe naga passes out from your attacks, allowing you take a moment to brush off some of the sparkly colorful dust from her wings for potential use later. Not quite sparkly of colorful when in your hands, you can at least ball it up into a nice suppository.');
  } else if (gs.enemyID === 307) {
    ...
    if (gs.defeatedSuccubus === false) {
      ...
      gs.currentDungeon = 1008;
    }
  } else {
    textLP('\r\rYou have made the ' + enemyName() + ' pass out from all the pain.');
  }
}

// ── special KO lose text ───────────────────────────────────────────────────

export function specialKOLose(): void {
  if (gs.currentState === 2) {
    if (gs.enemyID === 303) {
      textLP('\r\rJust as you\'re about to pass out, you see the octopus girl lean over your body. She wears a disappointed expression, finding you were\'t strong enough for what she was looking for. Shrugging, she jumps back into the ocean, leaving you to yourself.');
    } else if (gs.enemyID === 304 || gs.enemyID === 305) {
      textLP('\r\rJust as you\'re about to pass out, you feel yourself beginning to grow. The effects of the bottle seems to wear off as you return to your normal height. Though not quite all the way...');
    } else if (gs.enemyID === 309) {
      textLP('\r\rThe succubus seems a bit surprised as you pass out. "Oops... I thought you could take more than that. Sorry~"');
    }
  }
}
...
export function enemyAttackImpl(): void {
  const attack = percent();
  ...
  if (gs.enemyID === 101) {
    if (attack <= 50) {
      ...
      if (gs.cockTotal > 0) {
        textLP(' ' + (gs.cockTotal > 1 ? 'your' : 'your') + ' ' + cockDesc() + ' cock' + plural(1) + '. It doesn\'t hurt much, but the venom that spills into you makes you feel strangely aroused and your cock' + plural(1) + ' swell' + plural(3) + ' within your ' + clothesBottom() + ', becoming permanently larger, while the venom continues to make it feel warm...');
        doLust(Math.floor((gs.cockSize * gs.cockSizeMod / 5) + (percent() / 10)), 1);
        // AS3 inherited bug fix: TS port dropped `cockChange(1, 0)`. Without
        // it, "your cock swells…becoming permanently larger" is a lie — the
        // appearance text never updates after repeated bites.
        cockChange(1, 0);
        gs.cockSnakeVenom += 5;
      } else if (gs.cockTotal < 1 && gs.vagTotal > 0) {
        if (gs.clitSize > 20 && percent() <= 5) {
          textLP('  ' + clitDesc() + ' clit' + plural(2) + '. You feel it swell and shift within your ' + clothesBottom() + ', your lips started to grow quite oddly as well...');
          // AS3 inherited bug fix: rare 5%-chance vag→cock conversion. TS
          // port dropped both the vagChange (lose vag) and the cockChange
          // (gain a cock sized off the swollen clit), so the player just got
          // arousal text with no actual transformation.
          vagChange(0, -1);
          cockChange(Math.ceil(gs.clitSize * 5 / 2), 1);
          doLust(Math.floor((gs.cockSize * gs.cockSizeMod / 5) + (percent() / 10)), 1);
        } else {
          textLP('  ' + clitDesc() + ' clit' + plural(2) + '. It doesn\'t hurt much, but the venom that spills into you makes you feel strangely aroused as your clit' + plural(2) + ' swell' + plural(4) + ' a little within your ' + clothesBottom() + ', becoming permanently larger, while the venom continues to make it feel warm...');
          gs.cockSnakeVenom += 5;
          gs.clitSize++;
          doLust(Math.floor(gs.clitSize / 5 + percent() / 10), 1);
        }
      } else {
        if (percent() <= 40) {
          textLP(' your groin. It doesn\'t hurt much, but you feel a little odd...');
          // AS3 inherited bug fix: TS port dropped `cockChange(1, 0)` — the
          // venom should sprout/grow a cock on a no-genitals victim too.
          cockChange(1, 0);
  ...
  if (gs.enemyID === 102) {
    if (attack <= 30) {
      ...
      (moistCalc(1) > 11 && gs.cockTotal > 0) ||
      (moistCalc(2) > 11 && gs.vagTotal > 0) ||
      (gs.milkEngorgement > 200 && gs.milkEngorgementLevel > 1) ||
      (gs.udderEngorgement > 200 && gs.udderEngorgementLevel > 1 && gs.udders === true)
    )) {
      if (gs.milkEngorgement > 200 && gs.milkEngorgementLevel > 1) {
      ...
      } else if (gs.udderEngorgement > 200 && gs.udderEngorgementLevel > 1 && gs.udders === true) {
        ...
        doeHP(5 + Math.floor(percent() / 20));
      } else if (moistCalc(1) > 11 && gs.cockTotal > 0) {
        ...
        doeHP(5 + Math.floor(percent() / 20));
      } else if (moistCalc(2) > 11 && gs.vagTotal > 0) {
        textLP('\r\rThe whirling sand licks up ' + legDesc(1) + ' your ' + legDesc(2) + ', across your ' + vulvaDesc() + ' groin, and laps up some of the slick lubrication that spills from it, sucking the stuff back in and strengthening the devil\'s endurance.');
        doeHP(5 + Math.floor(percent() / 20));
      }
    } else if (attack <= 90) {
      const chance = percent();
      if (chance < 12 && gs.cockTotal > 0) {
        ...
        gs.cockSize--;
        doeHP(2);
      } else if (chance < 23 && gs.balls > 0 && gs.showBalls === true) {
        if (gs.ballSize < 2) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesBottom() + ' and softly drags across your ' + ballDesc() + ' scrotum. However, your balls are so puny that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
        ...
        if (gs.butt < 1) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesBottom() + ' and whisks across your ' + buttDesc() + ' rump. However, your butt is so flat that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
      ...
      } else if (chance < 45 && gs.vagTotal > 0) {
        if (gs.vulvaSize < 2) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesBottom() + ' and slips through your ' + vulvaDesc() + ' lips. However, your vulva is so tiny that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
      ...
      } else if (chance < 56 && gs.vagTotal > 0) {
        if (gs.clitSize < 2) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesBottom() + ' and whirls about your ' + clitDesc() + ' clit' + plural(2) + '. However, your button' + plural(2) + ' is so tiny that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
        ...
        if (gs.breastSize < 1) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesTop() + ' and gropes about your ' + boobDesc() + ' breasts. However, your chest is so flat that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
      ...
      } else if (chance < 78 && gs.udders === true) {
        if (gs.udderSize < 2) { textLP('\r\rSome of the devil\'s sand blow across your ' + udderDesc() + ' udder. However, your milk-bag is so small that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
      ...
      } else if (chance < 89 && gs.udders === true) {
        if (gs.teatSize < 3) { textLP('\r\rSome of the devil\'s sand licks across your ' + teatDesc() + ' teats. However, your bovine-nipples are so little that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
        ...
        if (gs.nippleSize < 2) { textLP('\r\rSome of the devil\'s sand creeps into your ' + clothesBottom() + ' and brushes over your ' + nipDesc() + 'nipples. However, your nipples are so little that there is hardly any moisture for the sand to take, doing nothing for the devil.'); }
        else {
      ...
      if (chance2 < 12 && gs.cockTotal > 0) {
        ...
        gs.cockSize -= 5;
        doeHP(10);
      } else if (chance2 < 23 && gs.balls > 0 && gs.showBalls === true) {
        if (gs.ballSize < 1) { textLP('balls. However, they\'re already so puny that the devil can\'t siphon any more moisture from them, proving a fruitless attack.'); }
        else {
          ...
          gs.ballSize -= 5;
          if (gs.ballSize < 0) { gs.ballSize = 0; }
          doeHP(10);
        }
      } else if (chance2 < 34) {
        if (gs.butt < 1) { textLP('butt. However, it\'s so flat that the devil can\'t siphon any more moisture from it, proving a fruitless attack.'); }
        else {
          ...
          gs.butt -= 5;
          if (gs.butt < 0) { gs.butt = 0; }
          doeHP(10);
        }
      } else if (chance2 < 45 && gs.vagTotal > 0) {
        if (gs.vulvaSize < 2) { textLP('vulva. However, your lips are so thin and tiny that the devil can\'t siphon any more moisture from them, proving a fruitless attack.'); }
        else {
          ...
          gs.vulvaSize -= 5;
          if (gs.vulvaSize < 1) { gs.vulvaSize = 1; }
          doeHP(10);
        }
      } else if (chance2 < 56 && gs.vagTotal > 0) {
        if (gs.clitSize < 2) { textLP('clit' + plural(2) + '. However, your button is so tiny that the devil can\'t siphon any more moisture from it, proving a fruitless attack.'); }
        else {
        ...
        if (gs.breastSize < 1) { textLP('chest. However, it\'s so flat that the devil can\'t siphon any more moisture from it, proving a fruitless attack.'); }
        else {
      ...
      } else if (chance2 < 78 && gs.udders === true) {
        if (gs.udderSize < 2) { textLP('udder. However, it\'s so small that the devil can\'t siphon any more moisture from it, proving a fruitless attack.'); }
        else {
      ...
      } else if (chance2 < 89 && gs.udders === true) {
        if (gs.teatSize < 3) { textLP('teats. However, they\'re so tiny that the devil can\'t siphon any more moisture from them, proving a fruitless attack.'); }
        else {
        ...
        if (gs.nippleSize < 2) { textLP('nipples. However, they\'re so tiny that the devil can\'t siphon any more moisture from them, proving a fruitless attack.'); }
        else {
  ...
  if (gs.enemyID === 201) {
    if (attack > 35 && attack <= 50) {
      ...
      doHP(-eDmg(6));
    } else if (attack > 50 && attack <= 70 && gs.lust > 15) {
      ...
      doLust(Math.floor(gs.lib / 3), 1);
      doeLust(-Math.floor(percent() / 10 + gs.eSen / 2));
    } else if (attack > 70 && gs.gender !== 0 && gs.gender !== 1) {
      ...
      doLust(Math.floor(percent() / 10 + gs.lib / 4), 1);
    } else {
  ...
  // Horny Felin
  if (gs.enemyID === 301) {
    if (attack > 30 && attack <= 60 && gs.eLust > 20) {
      ...
      doLust(Math.floor(percent() / 10 + gs.lib / 3), 1);
      doeLust(-Math.floor(percent() / 10 + gs.eSen / 2));
    } else if (attack > 60 && attack <= 85 && gs.eLust > 10) {
  ...
  if (gs.enemyID === 303) {
    if (attack > 30 && attack <= 55) {
      ...
      textLP('\r\rOne of her tentacles lashes out at you and whips up ' + legDesc(1) + ' your ' + legDesc(2) + '. It smarts a bit, but it seems as though she intentionally hit you with the softer underside. The suction cups tear through your ' + clothesBottom() + ' and stick to ');
      if (gs.vagTotal > 0) {
        ...
        doHP(-5);
        gs.clitSize++;
      } else if (gs.vagTotal < 1 && gs.cockTotal > 0) {
        textLP('your ' + cockDesc() + ' cock' + plural(1) + '. You can feel fluid seep from the cups and into your cock' + plural(2) + ', arousing you further. The tentacle wraps around, tugging again and again at your erection, as through she were masturbating you. Although, from the focused expression on her first, she seems to simply be trying to wrench herself free after the attack. Not that it arouses you any less...');
        doLust(Math.floor(10 + (percent() / 10) + gs.lib / 5), 1);
      } else {
    ...
    } else if (attack > 80 && gs.attireBot === -1) {
  ...
  if (gs.enemyID === 304) {
    if (attack > 30 && attack <= 55) {
      ...
      doHP(-eDmg(15));
      doLust(Math.floor((percent() / 10) + gs.lib / 10), 1);
      ...
      if (gs.breastSize >= 100 && gs.breastSize / (gs.tallness || 1) > 2.4) {
        ...
        if (gs.tail > 1) { textLP('his hand yanking your ' + tailDesc() + ' high,'); }
  ...
  if (gs.enemyID === 306) {
    if (attack > 25 && attack <= 45) {
      ...
      if (gs.eggLaying > 0 && gs.vagTotal > 0) {
        textLP('\r\rYour womb feels more active, like your next egg is ready to come out faster...');
        gs.eggRate++;
        gs.eggceleratorTime += 10;
        gs.eggceleratorDose++;
        if (gs.eggceleratorDose > (8 + Math.ceil(percent() / 20))) {
          textLP('A little too fast...\r\rYour belly lets out a groan as you feel the fresh egg already press against your lips, demanding its way out. In the midst of battle, you squat where you stand, already in the process of laying.');
          textLP('\r\rIt doesn\'t take long before your ' + legDesc(2) + ' finally give' + ' out and you fall into the mess, eggs still slipping through your slit. It doesn\'t take long for your body to burn through its production, your loins crying out in arousal as the laying slows. And once the last egg slips out, the naga licks her lips and closes in on the defenseless prey...');
          doLust(1000, 0);
        ...
        for (let i = 0; i < gs.pregArray.length; i += 5) {
          if (gs.pregArray[i] === true) { gs.pregArray[i + 3] += 10; }
        }
      } else if (gs.gender !== 0) {
  ...
  if (gs.enemyID === 307) {
    if (attack <= 25) {
      ...
      if (percent() > gs.str) {
        ...
        doLust(Math.floor(gs.eLust / 2), 1);
        gs.eLust -= 20;
        if (gs.currentState === 2) { enemyAttackImpl(); }
      } else {
  ...
  if (gs.enemyID === 308) {
    if (attack <= 23) {
      ...
      doHP(-eDmg(4));
      doLust(Math.floor(percent() / 10 + gs.sen / 5), 1);
    } else if (attack > 46 && attack <= 61 && gs.eHP < 85) {
      ...
      doHP(-eDmg(2));
    } else if (attack > 61 && attack <= 76 && gs.eLust > 50) {
  ...
  if (gs.enemyID === 309) {
    if (attack <= 25) {
      if (gs.breastSize / (gs.tallness || 1) < 1.07) {
        ...
        if (percent() < gs.ment) { textLP('The woman tries to tempt you, but you manage to resist.'); }
        else {
      ...
      const chance = percent();
      if (chance <= 20 && gs.hair > 0 && gs.hairLength < 10) {
        textLP('\r\r"You know what helps make a slut look good?" She steps up behind you and runs her fingers through your hair. A tingly sensation envelops your skull');
        if (percent() < gs.ment) { textLP(', but you manage to resist and interrupt her efforts.'); }
        else {
      ...
      } else if (chance <= 60 && gs.cockTotal > 0 && gs.cockSize > 2) {
...
// ── doRape — player rapes enemy on battle win ──────────────────────────────

export function doRape(): void {
  // Enemy 1 (test)
  if (gs.enemyID === 1) {
    if (gs.gender === 1) textL("You stick your cock in the test enemy's butt and cum.")
    if (gs.gender === 2) textL("You sit on the test enemy's cock until you cum.")
    if (gs.gender === 3) textL("You stuff your cock in the test enemy's butt, then sit on its cock and cum.")
  }
  ...
  if (gs.enemyID === 201) {
    ...
    doLust(-Math.floor(gs.sen / 2), 2, 2)
  }
  ...
  if (gs.enemyID === 202) {
    ...
    if (gs.showBalls === true) { textLP(' Your ' + ballDesc() + ' testicles eventually come to a rest upon the wolf\'s belly, with your own ' + cockDesc() + ' cock' + plural(1) + ' bobbing above.') }
    ...
    doLust(-Math.floor(gs.sen / 2), 2, 5)
  }

  // Felin in Heat (301)
  if (gs.enemyID === 301) {
    const chance = percent()
    if (gs.gender === 1 || (percent() <= 50 && gs.gender === 3)) {
      ...
      if (gs.cockSize * gs.cockSizeMod > eVagLimit(40)) textLP('\r\rHowever, she cries out in pain and disappointment, your ' + cockDesc() + ' cock far to big to fit inside without tearing her apart. So, instead, you slip it between her legs, rubbing it across her stiff clit and through her loincloth and over her belly as you ')
      else if (gs.cockSize * gs.cockSizeMod > eVagLimit(24)) textLP('\r\rShe cries out in pain and pleasure as your ' + cockDesc() + ' cock stretches her thirsting cunt wide, pushing her bikini in until it tears from your path. She pushes her rump back towards you, trying her best to devour the hot rod all the way to its hilt as you ')
      else textLP('\r\rShe cries out in ecstacy as her hungry cunt devours your ' + cockDesc() + ' cock, along with her own bikini until it tears from your path. She takes it all the way in to your hilt as you ')
      ...
      if (gs.cockSize * gs.cockSizeMod > eVagLimit(40)) textLP("across her chest while she tries to lap up the spray with her tongue. You pull back, leaving her a mess.")
      else textLP('into her womb, making her let out a long and joyful mewl. Before she can become too content, you pull out of her with a slurp, your cum still dribbling from her used sex.')
      cumAmount()
    } else {
      ...
      if (gs.cockTotal > 0 && cumAmount() > 0) textLP(' You pull your ' + cockDesc() + ' cock' + plural(1) + ', letting the engorged length' + plural(1) + ' bounce against her belly and hump through her many breasts.')
  ...
  // Drunken Equan (302)
  if (gs.enemyID === 302) {
    if (gs.gender === 2 || (percent() <= 50 && gs.gender === 3)) {
      ...
      if (vagLimit() < 32) {
        ...
        textLP('\r\rYou plunge down onto his giant man-meat with ' + oneYour(2) + ' ' + vulvaDesc() + ' cunt' + plural(2) + '. It fills you up so much you grin with a drool. You hug him close, your ' + boobDesc() + ' tits pressing against his firm chest, making the thing inside you even harder.')
      }
      ...
      if (vagLimit() < 32) {
        ...
        if (gs.cockTotal > 0 && cumAmount() > 0) textLP(', with yours joining in the volley,')
        ...
        if (gs.cockTotal > 0 && cumAmount() > 0) textLP(', while yours sprays across his chest and face')
    ...
    doLust(-Math.floor(gs.sen / 2), 2, 1, 5)
  }

  // Octopus Girl (303)
  if (gs.enemyID === 303) {
    if (gs.gender === 1 || gs.gender === 3) {
      ...
      if (gs.vagTotal > 1 && gs.vagTotal < 9) textLP('\r\rOne tentacle for each of your pussies, they softly dive in, as far as they can go before they feel you begin to stretch. They slip in and out as you hump the girl, quickly driving you towards climax.')
      else if (gs.vagTotal === 1) textLP('\r\rA tentacle softly slips into your pussy, diving in as far as it can go before it feels you begin to stretch. It slips in and out as you hump the girl, quickly driving you towards climax.')
      else if (gs.vagTotal > 8) textLP('\r\rTaking turns at each of your pussies, the tentacles softly dive in, as far as they can go before they feel you begin to stretch. They slip in and out as you hump the girl, quickly driving you towards climax.')
    ...
    doLust(-Math.floor(gs.sen / 2), 2, 1, 2)
  }

  // Little Big Bunny-Man (304)
  if (gs.enemyID === 304) {
    if (gs.vagTotal > 0 && vagLimit() > 80) {
      ...
      if (gs.gender === 1) textLP(' ' + cockDesc() + ' cock' + plural(1))
      if (gs.gender === 2) textLP(' ' + vulvaDesc() + ' cunt' + plural(2))
      if (gs.gender === 3) textLP(' ' + cockDesc() + ' cock' + plural(1) + ' and ' + vulvaDesc() + ' cunt' + plural(2))
  ...
  // Little Big Bunny-Girl (305)
  if (gs.enemyID === 305) {
    if (gs.cockSize * gs.cockSizeMod > 140 || gs.cockTotal * gs.cockSize * gs.cockSizeMod > 280) {
      ...
      if (gs.tallness < 180) {
        ...
        if (gs.knot === true) textLP(' Your swelling knot' + plural(1) + ' pop in and out of her relatively tight cun again and again, causing her to let out an ecstatic shriek as she\'s stretched obscenely.')
        ...
        if (gs.knot === true) textLP(', your knot' + plural(1) + ' making a lewd popping sound as it ejects from her stretched pussy, eliciting one last yelp of ecstasy')
        textLP('.')
      }
    } else {
  ...
  if (gs.enemyID === 307) {
    ...
    if (gs.vagTotal > 0) {
      ...
      if (gs.showBalls === true) textLP(' while your ' + ballDesc() + ' balls slap against his head-sized testicles')
    ...
    doLust(-Math.floor(gs.sen / 2), 2, 2, 5)
  }

  // Freaky Little Girl (308)
  if (gs.enemyID === 308) {
    ...
    if ((gs.cockTotal > 0 && gs.cockSize * gs.cockSizeMod < eVagLimit(24)) || (gs.vagTotal > 0 && gs.clitSize < 60)) {
      textLP('Pulling ' + pullUD(2) + ' your ' + clothesBottom() + ', you angle ')
      if (gs.cockTotal > 0 && gs.cockSize * gs.cockSizeMod < eVagLimit(24)) textLP(oneYour(1) + ' ' + cockDesc() + ' cock' + plural(1))
      ...
      if (gs.cockTotal > 0 && gs.cockSize * gs.cockSizeMod < eVagLimit(24)) {
  ...
  if (gs.enemyID === 309) {
    ...
    if (gs.cockTotal > 0 || (gs.vagTotal > 0 && gs.clitSize > 20)) {
      textLP('\r\rYou plunge ')
      if (gs.cockTotal > 0) textLP(oneYour(1) + ' ' + cockDesc() + ' cock' + plural(1))
      else textLP(oneYour(2) + ' ' + clitDesc() + ' clit' + plural(2))
      textLP(' into her awaiting entrance, pushing in deep.')
      if (gs.cockTotal > 0 && gs.cockSize * gs.cockSizeMod > 48) textLP(' Amazingly, her sensual lips stretch wider and wider, taking in your whole length until your cock pushes back out against her belly below, her body easily taking you in and hugging you tightly with her warmth.')
      else if (gs.cockTotal < 1 && gs.clitSize > 120) textLP(' Amazingly, her sensual lips stretch wider and wider, taking in your whole length until your clitoris pushes back out against her belly below, her body easily taking you in and hugging you tightly with her warmth.')
...
// ── doGetRaped — enemy rapes player on battle loss ─────────────────────────

export function doGetRaped(): void {
  // Test enemy
  if (gs.enemyID === 1) {
    if (gs.gender === 1) textL("The test enemy pokes its cock in your butt and cums.")
    if (gs.gender === 2) textL("The test enemy pokes its cock in your vagina and cums.")
    if (gs.gender === 3) textL("The test enemy pokes its cock in your vagina and cums while stroking you off.")
  }
      ...
      if (pregCheck(1)) {
        ...
        for (let i = 0; i < gs.pregArray.length; i += 5) {
          if (gs.pregArray[i] === false) {
            gs.pregArray[i] = true
            gs.pregArray[i + 1] = 503
            gs.pregArray[i + 2] = 240
            gs.pregArray[i + 3] = 150
            break
          }
        }
        gs.cockSnakePreg += 50
  ...
  if (gs.enemyID === 201) {
    ...
    if (vagLimit() < 32) {
      ...
      textLP('but your gaping cunt is more than enough to handle it,')
    }
  ...
  if (gs.enemyID === 301) {
    ...
    changeTop(-1)
    if (gs.lactation > 0) textLP(', delighting in the taste of your milk')
    ...
    if (gs.gender === 1 || gs.gender === 3) {
      ...
      if (gs.cockSize * gs.cockSizeMod > eVagLimit(40)) textLP(' She attempts to stand and squat down upon it, but, to her dismay, she can\'t seem to push it into her hungry pussy, even though she tried until tears welled up in her eyes from the painful stretching. Instead, she settles for hugging and humping the ' + cockDesc() + ' thing, grinding her own stiff little erection into it, her feminine juices spilling down its length and over your body.')
      else if (gs.cockSize * gs.cockSizeMod > eVagLimit(20)) textLP(' She mewls and mrowls in pain as she stands up and squats down upon it. It spreads her so wide that it hurts her so much, yet she doesn\'t care, so desperate to fuck. Tears roll down her cheeks, but her mouth yawns wide with erotic joy, being filled so much.')
      ...
      if (cumAmount() > 550 && gs.cockSize * gs.cockSizeMod <= eVagLimit(40)) textLP(' She purrs loudly as she feels your cum explode within her, utterly pleased as it fills her so much that it squirts back out of her pussy with a loud lewd noise.')
      else if (gs.cockSize * gs.cockSizeMod > eVagLimit(40)) textLP(' She purrs hungrily as she laps at you cum as it sprays about the both of you, her face absolutely delighted in having her fur matted with the stuff.')
      ...
      textLP('\r\r')
      if (gs.knot === true && gs.cockSize * gs.cockSizeMod <= eVagLimit(40)) textLP('With a high-pitched squeak, she pulls off of your knot without thinking. She rubs her poor cunny from the pain, only to roll her eyes up into her head with the pleasant masturbation. ')
    }
    if (gs.gender === 2) {
      ...
      if (gs.clitSize > 25) {
        ...
        textLP(' She glides back up your chest and presses her sloppy cunt to yours, grinding the two together.')
      }
      textLP(' Her hips speed up faster and faster, wildly working her pussy so much that the slick liquids spilling from her gurgle and churn into a bubbly mess. You too find her efforts to be extremely effective, your body quaking along with her.')
    }
  ...
  if (gs.enemyID === 302) {
    ...
    if (gs.gender === 1 || (gs.gender === 3 && percent() <= 50)) {
  ...
  // Octopus Girl (303) — gender 1
  if (gs.enemyID === 303 && gs.gender === 1) {
  ...
  // Octopus Girl (303) — gender 2
  if (gs.enemyID === 303 && gs.gender === 2) {
    ...
    if (!pregCheck(1)) {
      ...
      if (gs.vagSize < 20) { textLP(' The object is so big that it stretches your poor cunt' + plural(2) + ' even larger, making sure there\'s enough room.'); gs.vagSize += 2 }
      ...
      for (let i = 0; i < gs.pregArray.length; i += 5) {
        if (gs.pregArray[i] === false) { gs.pregArray[i] = true; gs.pregArray[i + 1] = 1000; gs.pregArray[i + 3] = 180; break }
      }
    }
  }

  // Octopus Girl (303) — gender 3 summary handled same as gen 1 then gen 2
  if (gs.enemyID === 303 && gs.gender === 3) {
    // Same as gender 1 tentacle oral then gender 2 spawn — abbreviated per source
    doLust(-Math.floor(gs.sen / 2), 2, (pregCheck(1)) ? 2 : 1)
  }
  if (gs.enemyID === 303 && (gs.gender === 2 || gs.gender === 3) && pregCheck(1)) {
    doLust(-Math.floor(gs.sen / 2), 2, 2)
  } else if (gs.enemyID === 303) {
    doLust(-Math.floor(gs.sen / 2), 2, 1)
  }
  ...
  if (gs.enemyID === 304) {
    if (gs.vagTotal > 0 && vagLimit() > 80) {
      ...
      if (gs.tallness < 160) textLP(' You can feel your belly distend, your vaginal flesh stretching out within to match the shape of his cock.')
      if (gs.tallness < 80) textLP(' So much so that your vision becomes blocked by your own ' + skinDesc() + ', the massive erection squeezing between your face and his body.')
      ...
      if (gs.cockTotal > 0) textLP(', barely noticing the extra stickiness upon his fur that rubs against your belly as your own erection' + plural(1) + ' explode' + plural(3))
  ...
  if (gs.enemyID === 307) {
    ...
    if (gs.vagTotal > 0 && vagLimit() > 72) textLP(' He proceeds to stuff his massive schlong into ' + oneYour(2) + ' cunt' + plural(2) + ' with a loud slurp, pushing in until he can see his own cock bulge out from your belly.')
    else if (gs.vagTotal > 0) textLP(' With your cunt' + plural(2) + ' obviously too small, he proceeds to stuff his massive schlong into your ass with a loud slurping squish.')
    ...
    if (gs.cockTotal > 0) { textLP(', your cock' + plural(1) + ' launching ribbons over yourself while ' + plural(11) + ' bounce' + plural(3) + ' against you'); cumAmount() }
  ...
  if (gs.enemyID === 308) {
    ...
    if (gs.vagTotal > 0) { doImpregnate(enemyBaby()); doImpregnate(enemyBaby()) }
    doLust(-Math.floor(gs.sen / 2), 2, 2)
  }
  ...
  if (gs.enemyID === 309) {
    ...
    if (gs.cockTotal > 0 || (gs.vagTotal > 0 && gs.clitSize > 20)) {
      ...
      if (gs.cockTotal > 0 && gs.cockSize * gs.cockSizeMod > 48) textLP(" Amazingly, her sensual lips stretch wider and wider, taking in your whole length until you can see your own cock pushing back out against her belly, her body easily taking you in and hugging you tightly with her warmth.")
      else if (gs.cockTotal < 1 && gs.clitSize > 120) textLP(" Amazingly, her sensual lips stretch wider and wider, taking in your whole length until you can see your own clitoris pushing back out against her belly, her body easily taking you in and hugging you tightly with her warmth.")
      ...
      if (gs.cockTotal > 0) { cumAmount(); textLP(' Your spunk blows within her, filling her insides and drooling back out to your crotch. She doesn\'t seem to retain much of your cum, though.') }
    } else {
    ...
    stats(-2, -1, 2, 0)
    gs.submissiveFetish += 0.05
    gs.dominantFetish -= 0.05
    gs.tallness -= 2
    gs.body -= 2
    doLust(-Math.floor(gs.sen / 2), 2, 1, 2)
  }
  ...
  if (gs.inDungeon === true) gs.inDungeon = false
  gs.currentState = 1
...
// Battling.ts now imports `enemyAttackImpl` directly (the cycle is safe
// because both modules only touch each other inside function bodies). The
// previous gs-attached dispatch is no longer needed; keep the empty
// register stub so older save files / dev tools that look for it don't
// crash on startup.
export function registerEnemyAttack(): void { /* no-op */ }
