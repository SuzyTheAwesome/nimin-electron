// Ported from Events/Cave.as
// Note: exported as both doOldCave and doCave for compatibility
...
import { eventSelect, triggerOldCaveDescent } from '../screens/Exploration.ts'
...
import { itemAdd, checkItem } from '../content/Items.ts'
...
import {
  vulvaDesc, cockDesc, buttDesc, hipDesc, boobDesc, nipDesc,
  legDesc, legVerb, clitDesc, plural, oneYour
} from '../content/Descriptions.ts'
import { clothesTop, clothesBottom, pullUD, changeTop, changeBot, currentClothes } from '../content/Clothes.ts'
import { regionChange } from '../screens/EventUtilities.ts'

export function doOldCave(): void {
  const chance = eventSelect('Old Cave')
  if (chance === 1) {
      ...
      gs.doListen = function(): void {
        if (gs.buttonChoice === 6) {
          ...
          if (gs.tallness <= 18) {
            ...
            if (gs.ment >= 30) textLP(' You grab your clothes, finding them to be a little small for you, but they\'re otherwise fine.')
            ...
            if (gs.gender === 1 || (gs.gender === 3 && percent() <= 50)) {
              ...
              if (gs.cockSize * gs.cockSizeMod > 400) {
                ...
                gs.doListen = function(): void {
                  ...
                  if (gs.ment >= 30) textLP(' Especially as you re-don your now too-small clothes, feeling the outfit hug your body tightly and definitely needing some readjusting when you get back to town...')
            ...
            } else if (gs.gender === 2 || gs.gender === 3) {
              ...
              gs.doListen = function(): void {
                ...
                if (pregCheck(1) && percent() <= 60 && gs.vagSize * gs.vagSizeMod > 56) {
                  ...
                  for (let i = 0; i < gs.pregArray.length; i += 5) {
                    if (gs.pregArray[i] === false) {
                      gs.pregArray[i] = true
                      gs.pregArray[i + 1] = percent() <= 50 ? 1001 : 1002
      ...
      gs.doListen = function(): void {
        if (gs.buttonChoice === 6) {
          ...
          if (gs.attireTop === gs.attireBot) {
            textLP(' Your ' + clothesTop() + ' plummets down your ' + boobDesc() + ' chest. You catch it with your hands, but soon the material bundles over your arms that you can no longer maintain your hold. When you let go, the ' + clothesTop() + ' simply sits there, piled around you.')
          } else {
          ...
          textLP('\r\rYou climb out from your overgrown clothes, completely nude. Though you doubt anybody could see you if they passed by, considering you\'re now a fraction of the height you were before, maybe a whole ' + Math.floor(gs.tallness / 12) + ' inches tall.')
          gs.tallness -= 2
          doNext()
          gs.doListen = function(): void {
            const roll = percent()
            if (roll <= 40) {
              ...
              if (gs.tallness <= 216) textLP(' he towers over you, pleased with the result.')
              else textLP(' you seem to still be on par with him, though he seems pleased you\'re much more manageable than before.')
              doNext()
              gs.doListen = function(): void {
                gs.enemyID = 304
                gs.currentState = 2
              ...
              if (gs.tallness <= 216) textLP(' she towers over you, pleased with the result.')
              else textLP(' you seem to still be on par with her, though she seems pleased you\'re much more manageable than before.')
              doNext()
              gs.doListen = function(): void {
                gs.enemyID = 305
                gs.currentState = 2
    ...
    if (checkItem(254)) {
      ...
      gs.doListen = function(): void {
        if (gs.buttonChoice === 6) {
          ...
          gs.inDungeon = true
          // Calls into Exploration's dungeon descent handler
          triggerOldCaveDescent()
        } else {
