...
import { checkMagicItem } from '../content/Items.ts'
...
import { vagLimit, moistCalc, cumAmount } from '../core/Calculations.ts'
import { bustRatio } from '../content/Breasts.ts'
import { vagChange } from '../systems/Transformations.ts'
...
import {
  vulvaDesc, cockDesc, boobDesc, nipDesc, skinDesc, bellyDesc,
  legDesc, buttDesc, plural, oneYour
} from '../content/Descriptions.ts'
import { clothesTop, clothesBottom } from '../content/Clothes.ts'

export function doDesert(): void {
  const chance = eventSelect('Desert')
  ...
  if (chance === 1) {
    ...
    gs.doListen = function(): void {
      if (gs.buttonChoice === 6) {
        ...
        if (roll <= 15) {
          ...
          if (gs.cockTotal > 0 && gs.vagTotal < gs.cockTotal + 1) {
            ...
            vagChange(0, gs.cockTotal - gs.vagTotal + 1)
    ...
    if (gs.pheromone > 0 && checkMagicItem()) {
        ...
        if (gs.buttonChoice === 6) {
          ...
          gs.doListen = function(): void {
            ...
            if (gs.cockTotal > 0) {
              ...
              if (gs.cockSize * gs.cockSizeMod < 36) textLP('easily engulfing your whole length')
              else if (gs.cockSize * gs.cockSizeMod < 66) textLP('her pussy squeezing around your thick girth as it seems to barely manage to swallow your entire length')
              ...
              if (gs.knot) textLP(', making her yelp in suprise as your swollen knot stretches her for a moment and pops out.')
              ...
              gs.silPreg = 1
            } else if (gs.vagTotal > 0 && vagLimit() > 28) {
              ...
              if (gs.tallness / 2 < gs.vagSize * gs.vagSizeMod) textLP('pushing your belly out from within, ')
              ...
              if (gs.vagTotal > 0) textLP('\r\rNoticing she is far too large for you and still having the sense to not hurt you,')
              ...
              if (gs.udders) textLP(' smearing all over your udder.')
            ...
            gs.doListen = function(): void {
              ...
              if (gs.cockTotal > 0) textLP(' She then looks down at her belly and rubs it with a hand, relishing the feeling of your seed in her womb. "That was a lot more... fulfilling than I had imagined it would be." A glow seems to fill her expression as she croons over it.')
      ...
      if (gs.buttonChoice === 6) {
        ...
        if (gs.currentZone === 6) {
          if (percent() <= 50) {
            ...
            regionChange(4)
            if (!gs.foundSizCalit) gs.foundSizCalit = true
            gs.hrs = 4
            doEnd()
          }
        } else if (gs.currentZone === 2 || gs.currentZone === 4) {
          ...
          regionChange(6)
          if (!gs.foundOviasis) gs.foundOviasis = true
          gs.hrs = 4
          doEnd()
        }
      } else {
    ...
    if (gs.hour > 8 && gs.hour <= 20) {
      ...
      if (gs.skinType === 3) {
      ...
      } else if (gs.milkEngorgement > 400 && gs.milkEngorgementLevel > 2) {
      ...
      } else if (gs.udderEngorgement > 400 && gs.udderEngorgementLevel > 2 && gs.udders) {
      ...
      } else if (moistCalc(1) > 7 && gs.cockTotal > 0) {
      ...
      } else if (moistCalc(2) > 7 && gs.vagTotal > 0) {
        ...
        gs.exhaustion += 5
        stats(0, 0, -1, 0)
        doHP(-Math.floor((30 + Math.floor(gs.str / 2) + gs.HPMod) / 6))
      }
      gs.hrs = 2
      doEnd()
    } else {
      ...
      if (gs.skinType === 2) {
        ...
        stats(-1, 0, 0, 0)
        doHP(-Math.floor((30 + Math.floor(gs.str / 2) + gs.HPMod) / 3.5))
      }
      gs.hrs = 2
      doEnd()
    }
