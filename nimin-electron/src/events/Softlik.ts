// Ported from Events/Softlik.as
import { gs } from '../core/GameState.ts'
import { textL, textLP } from '../ui/TextRenderer.ts'
import { buttonConfirm, viewButtonOutline, viewButtonText, buttonWrite } from '../ui/ButtonManager.ts'
import { eventSelect } from '../screens/Exploration.ts'
import { doEnd } from '../core/GameEngine.ts'
import { itemAdd, checkItem, loseManyItem } from '../content/Items.ts'
import { doCoin } from '../systems/StatChanges.ts'

export function doSoftlik(): void {
  const chance = eventSelect('Softlik')
  if (chance === 1) {
    ...
    if (!gs.knowMaleEnhance) {
      buttonWrite(6, 'Buy recipe')
      viewButtonOutline(1, 0, 1, 0, 0, 1, 0, 0, 0, 0, 1, 0)
      viewButtonText(1, 0, 1, 0, 0, 1, 0, 0, 0, 0, 1, 0)
    }
    ...
    if (gs.gender === 2) textLP(' Or in your case, your husband or boyfriend?')
    ...
    if (!gs.knowMaleEnhance) textLP(' Or, for the low low price of 200 coins, I can give you the recipe to making them.')
    ...
    if (checkItem(211)) {
      ...
      gs.doListen = function(): void {
        if (gs.buttonChoice === 6) {
          if (gs.coin < 100) {
            textL('"Sorry, I\'m afraid you don\'t quite have enough coins. Maybe another time." He trudges off down the street.')
            gs.hrs = 1
            doEnd()
          } else {
