// Ported from Events/Tieden.as
import { gs } from '../core/GameState.ts'
import { textL, textLP } from '../ui/TextRenderer.ts'
import { viewButtonOutline, viewButtonText, buttonWrite, buttonConfirm } from '../ui/ButtonManager.ts'
...
import { milkAmount } from '../core/Calculations.ts'
import { pregCheck } from '../systems/Pregnancy.ts'
import { knotholeMain } from '../screens/EventUtilities.ts'
import { nipDesc, bellyDesc, skinDesc, vulvaDesc } from '../content/Descriptions.ts'
import { currentClothes } from '../content/Clothes.ts'

export function doTieden(): void {
  const chance = eventSelect('Tieden')
  if (chance === 1) {
      ...
      if (gs.buttonChoice === 6) {
        ...
        gs.doListen = function(): void {
          if (gs.buttonChoice === 6) {
            ...
            if (gs.pregnancyTime >= 400) textLP('climbs')
            else textLP('rolls')
            ...
            gs.nipplePlay += 6
            doLust(-Math.floor(gs.lib / 2), 2, 2)
            doEnd()
            gs.doListen = function(): void {
              ...
              if (pregCheck(0)) {
                for (let i = 0; i < gs.pregArray.length; i += 5) {
                  if (gs.pregArray[i] === true) gs.pregArray[i + 3] += 20
                }
              }
              itemAdd(248)
              gs.hrs = 3
