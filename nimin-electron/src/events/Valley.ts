...
import { doLust, doSexP, doHP } from '../systems/StatChanges.ts'
import { stats } from '../systems/StatSetup.ts'
import { bustRatio } from '../content/Breasts.ts'
import { skinDesc, legDesc } from '../content/Descriptions.ts'
import { clothesTop } from '../content/Clothes.ts'

function statueGreenMist(): void {
  ...
  if (gs.ment > 60) { textLP(' You have a feeling that the statue is trying to teach you how to really pleasure a woman...') }
  gs.fertilityStatueCurse = 48
  gs.minLust += 10
  doLust(0, 0)
...
function statueRevealEntrance(): void {
...
function showSubButtons(): void {
  const showCockCarv = checkItem(207) ? 1 : 0
  const showMilk = (checkItem(500) || checkItem(501)) ? 1 : 0
  viewButtonOutline(1, 0, 1, 0, 0, 0, 0, 1, showCockCarv, 0, showMilk, 0)
  viewButtonText(1, 0, 1, 0, 0, 0, 0, 1, showCockCarv, 0, showMilk, 0)
  buttonWrite(1, 'Lick')
  buttonWrite(3, 'Poke')
...
function doMouthSub(): void {
  ...
  gs.doListen = function(): void {
    ...
    if (gs.buttonChoice === 11) {
      ...
      statueRevealEntrance()
      if (checkItem(500)) { loseManyItem(500, 1) } else { loseManyItem(501, 1) }
      gs.hrs = 3
      doEnd()
    }
  ...
  gs.doListen = function(): void {
    ...
    if (gs.buttonChoice === 11) {
      ...
      if (!gs.knowBazoomba) {
        ...
        gs.knowBazoomba = true
      } else {
        textLP(' However, it\'s completely empty. You already took the recipe that was in here before, so there\'s nothing left within.')
      }
      textLP('\r\rThen, a glint of light obscures your view for just a moment and when it\'s gone you find yourself outside of the valley. The statue must have teleported you, preventing you from trying anything else. For now, you head back to town.')
      gs.hrs = 2
      if (checkItem(500)) { loseManyItem(500, 1) } else { loseManyItem(501, 1) }
...
function doPussySub(): void {
  ...
  gs.doListen = function(): void {
    ...
    if (gs.buttonChoice === 11) {
      ...
      gs.pregArray.push(true, 504, 208, 200, 0)
      if (checkItem(500)) { loseManyItem(500, 1) } else { loseManyItem(501, 1) }
      gs.hrs = 2
      doEnd()
    }
...
export function doValley(): void {
  const chance = eventSelect('Valley')
  ...
  if (chance === 1) {
    ...
    gs.doListen = function(): void {
      if (gs.buttonChoice === 1) { doMouthSub() }
      if (gs.buttonChoice === 3) { doEarSub() }
      if (gs.buttonChoice === 6) { doBreastSub() }
      if (gs.buttonChoice === 10) { doPussySub() }
