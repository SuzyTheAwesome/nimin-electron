// Ported from EventUtilities.as
import { gs } from '../core/GameState.ts'
import { el } from '../ui/UIManager.ts'
import { textL, textLP } from '../ui/TextRenderer.ts'
import { viewButtonOutline, viewButtonText, buttonWrite, buttonConfirm, doNext } from '../ui/ButtonManager.ts'
...
import { bodyDesc, legDesc, legWhere, plural, oneYour } from '../content/Descriptions.ts'
import { currentClothes } from '../content/Clothes.ts'

const ZONE_NAMES: Record<number, string> = {
  1: 'Softlik', 2: 'Firmshaft', 3: 'Tieden',
  4: "Siz'Calit", 5: 'Valley', 6: 'Oviasis',
}

export function regionChange(zone: number): void {
  gs.currentZone = zone
  const nameEl = el.currentRegion()
  if (nameEl) nameEl.textContent = ZONE_NAMES[zone] ?? 'Unknown'
}

export function sideEvent(which: number): void {
  gs.sideFocus = which
}

export function gibberish(): string {
  const chance = percent()
  if (chance <= 33) return '¤çÑ-| ÇôG+¦æ| EÆáÜaß pOƒ§· +îdvwqe 5dfÑ¯» º¤äÜ¦) ¼ÿæ¤h ·ƒ.'
  if (chance <= 66) return 'Gs¿ fdfƒæ d§ew ¤-ÿö fs¤£· ÖÅ¢¥¬ ¼«¦ds?'
  return 'Tas ªÜhf¤ ÄäÑse çåÅû¿ ÑÜñ?Äsd Ü¥¦»¦ƒ ¦ÜÆ+¿æ£ we¤ rgdA-d»¦± Ü+#A¤$¤-ò. Fi?¤çÑK)^¤2 ges nec ¤?+ÿ• ºñ¡as frtr.'
}

export function gibButt(): string {
  const chance = percent()
  if (chance <= 20) return 'Pk¿ºs'
  if (chance <= 40) return 'Ju£¥)'
  if (chance <= 60) return '§hdsa'
  if (chance <= 80) return 'Ö¦¤ÄT¦+'
  return 'Pancakes'
}

// ── Knothole ─────────────────────────────────────────────────────────────────

export function knotholeLeave(): void {
  textL('Finished with your time in the Knothole, you return to the fresh air of Tieden.')
  doEnd()
}

export function knotholeUpstairs(): void {
  ...
  gs.doListen = function(): void {
    ...
    if (gs.buttonChoice === 9) knotholeMain(false)
    if (gs.buttonChoice === 12) knotholeLeave()
  }
}

export function knotholeMain(entering: boolean): void {
  ...
  if (entering) {
    textL('Inside The Knothole, you come across an almost primal sight.\r\r')
  } else {
    textL('You return to the main floor of the Knothole, looking around to see what else you would like to do.\r\r')
  }
  ...
  gs.doListen = function(): void {
    if (gs.buttonChoice === 1) knotholeUpstairs()
    if (gs.buttonChoice === 6) {
